...
> It starts to look a bit clearer, doesn't it? Which kind of distribution is it?
...
#lines(0.29/sqrt(1:100))
```

> What is the characteristics of these values?
...
s.red = sd(red.obs)
s.red
s.green = sd(green.obs)
s.green

## critical value, using a t-distribution - This is the critical value for the t-distribution t95,n-1
t_95 = qt(0.975,df=N-1)

## confidence interval
m.red-t_95*s.red/sqrt(N) 
m.red+t_95*s.red/sqrt(N)

m.green-t_95*s.green/sqrt(N)
m.green+t_95*s.green/sqrt(N)
```

```{r}
## confidence interval using a Poisson distribution (lambda = exp.value = 14!)
red.low = m.red-t_95*sqrt(exp.value)/sqrt(N) 
red.low
red.high = m.red+t_95*sqrt(exp.value)/sqrt(N)
red.high

## Yes, the expected value is within the 95% CI.
## Here with a plot
# Define the color, green if within 95% CI, otherwise, red)
col = ifelse(exp.value < red.low | exp.value > red.high, "red", "darkgreen")
# 
plot(exp.value, 1, pch = 19, ylab = "")
segments(red.low, 1, red.high, 1, lwd = 2, col = col)
```
