...
We use again the diabetes dataset, and perform again the data cleaning. For a more in-depth explanation on the following data cleaning steps, see the last exercise sheet "01_Introduction", where this process is discussed in detail.

```{r, echo=FALSE, eval = TRUE}
dat = read.delim('https://tinyurl.com/y4fark9g') # load the data set

# set the row names using the column id
rownames(dat) = dat$id

# remove some columns: first, we determine which column index corresponds to the column names id, bp.2s, bp.2d, time.ppn
...
**Recommendation:** in order to avoid going through several excercise sheets in the future to perform the data cleaning, create now a R file where you save all of the code that we use to manipulate our data.  You can also save the environment containing all the needed variables as an .RData file or separately save a variable of interest. For example, the dataframe "dat" (after all cleaning steps):

```{r, eval= FALSE}
save(dat, file = "dat.RData") #save the data frame "dat" as an .Rdata file
```

Whenever you need it, you can load the dataframe using the function `load()`.

```{r, eval= FALSE}
load("dat.RData")
```

The numerical columns start at column index 4 and end at column index 14. We now create a new data frame called "dat.num" containing only the numerical columns:
...
We have looked at a dataset of diabetes patients and used functions to clean it up of missing data and visualize the cleaned up data.
...
Before you begin, think back of the lecture about mean, median and quantiles. Do you remember what these terms mean? How does an asymmetrical distribution influences mean and median? Think back to all these topics before you proceed.

We have already seen that the `summary()` and `quantile()` functions in R can compute the mean, median and quantiles of any given data. Let's now use functions in R that explicitly measure these values.
...
You will see that the distribution of the data is extremely squeezed due to **outliers** with very high or low values. We will need to make the data more homogeneous, such that our downstream analysis is not affected by these very large magnitude numbers.
...
Another widely used method for grouping observations is k-means clustering. Now we will cluster our dataset using k-means and explore the underlying structure of the data.
...
#### Exercise 1 - Quantiles and Histograms

1.  Compute the mean value of the cholesterol variable of "dat" and store it in the variable "m.chol".

2.  Compute the 1%, 10%, 25%, 50% (=median), 75%, 90%, 99% quantiles using the `quantile()` function, and using the argument `probs=c(0.01,0.1,0.25,0.5,0.75,0.9,0.99)`. Store these value in a new vector variable called "q.chol".
...
1.  Is there an association between "hip" and "waist" of the data frame "dat"? Use the `plot()` function to do a scatter plot of the **values** and of the **ranks** (determine the ranks with the `rank()` function). Compute both the **pearson** and **spearman** correlation values.

2.  Use the numerical dataframe "dat.num", and apply the function `cor()` **directly** on this data frame: `cor(dat.num)` . What happens? What is the output? Store the result of this command in a variable named "all.cor".  Plot a heatmap of these results using the `pheatmap()` function from the "pheatmap" package. Remember that you first have to activate this package using `library("pheatmap")`.

3.  Find the **highest** and **lowest** Pearson correlation value of the result from exercise 2.2. To which pair of variables do they correspond? Plot the corresponding scatter plots using `plot()`! *Hint*: Before finding the highest Pearson correlation value, use the `diag()` function to set all diagonal values (=1) of "all.cor" to NA.
...
##
cor(dat$hip,dat$waist,method='pearson')  ##Note: as Pearson is the default method, we do not need to explicitly indicate it!
...
plot(dat$hdl, dat$ratio)

max(all.cor) # This will give a value of 1, as all diagonal values are 1, which is trivial
...
plot(dat$weight, dat$waist)
```

#### Exercise 3

Let's use the new dataset "brca.exp" as an opportunity to apply the skills we learned in the previous exercise sheet. It's crucial that you keep exercising and repeating, otherwise you will easily forget your coding skills.

1.  Calculate the mean and median expression level and the standard deviation for each sample. Use the `apply(...)` function for this (you can check [this tutorial](https://www.datacamp.com/community/tutorials/r-tutorial-apply-family))

2.  Repeat the same for each gene.

3.  Plot the median expression values from all genes as an (a) histogram using the `hist()` function, and (b) density plot using the `density()` and `plot()` functions.
...
# All three at once
#
df_mean_median_sd <- data.frame(
  mean = apply(brca.exp,2,mean), 
  median = apply(brca.exp,2,median),
  sd = apply(brca.exp,2,sd))
df_mean_median_sd
...
3.  Calculate the pairwise correlations of these genes using `cor()` and plot again the heatmap with these genes using `pheatmap()` (see exercise 2.2).

```{r}
# 1. 
sd.genes = apply(brca.large, 1, sd)

# 2.
i.top = order(sd.genes, decreasing = TRUE)[1:200]
brca.top = brca.large[i.top,]

# 3. 
cor.top = cor(t(brca.top)) #transpose to calculate the correlation among genes
pheatmap(cor.top)
```
