...
<<<<<<< Updated upstream:04_ProbabilityDistributions_solutions.Rmd
...
>>>>>>> Stashed changes:archive/04_ProbabilityDistributions_solutions.Rmd
...
# Probability of 3 or more (beware: the argument is > 2, i.e. 3 or more)
...
3.  Suppose that the distribution of body height is described by a **normal distribution** with **mean = 1.75 m** and standard deviation **sd = 0.15**. What is the probability that someone is taller than 1.9 m? We can use the parameter **lower.tail** of the function `pnorm()` for this.   What is the probability that someone is smaller than 1.6 m?
>>>>>>> Stashed changes:archive/04_ProbabilityDistributions_solutions.Rmd
...
# or
pnorm(1.9,mean=1.75,sd=0.15,lower.tail=F)
# or
...
# or
pnorm(1.6,mean=1.75,sd=0.15,lower.tail=T)
# equivalent to 
pnorm(1.6,mean=1.75,sd=0.15)
# because per defaults, lower.tail is set to TRUE
# or
1-pnorm(1.6,mean=1.75,sd=0.15,lower.tail=F) # a bit more complicated view of the same thing.
```
...
z = dnorm(x,mean = 1.75, sd = 0.25)
plot(x,y,type='l',col='red',lwd=2);abline(v=c(1.6,1.9),lty=3,lwd=2);lines(x,z,col='blue',lwd=2)
```
...
hist(x,breaks=20)
#
## Poisson distribution: not possible, as var=mean !!
#
## Binomial distribution: not possible, as we must have var < mean 
## try instead with mean = 10, sd = 3
# mean = Lp = 10
# var = 10(1-p) = 3^2
# p = 0.1, L=100
y = rbinom(100,100,0.1)
hist(y,breaks=20)
```
...
hist(r1,breaks=seq(-10,30,by=.5),probability = TRUE,ylim=c(0,0.5),main="",xlab="") # Histogram visualization
...
r2 = rnorm( n=1000, mean=10, sd =1)
r3 = rnorm( n=1000, mean=20, sd =1)
hist(r1,breaks=seq(-10,30,by=.5),ylim=c(0,25),main="",xlab="")
hist(r2,breaks=seq(-10,30,by=.5),add=TRUE)
hist(r3,breaks=seq(-10,30,by=.5),add=TRUE)
lines(x,d1,col='red',type='l',lwd=4)
lines(x,d2,col='blue',lwd=4)
lines(x,d3,col='green',lwd=4)
```
...
l = c(1,10,100,1000)
sapply(l,function(x) {
  y = rpois(1000,lambda=x)
  hist(y, main = paste("lambda =", x))
})
```
...
A different way of visualizing the data which is more intuitive and clear is by plotting the densities of probabilities. We do the same as before (with the normal distribution) but now using the Poisson distribution.
