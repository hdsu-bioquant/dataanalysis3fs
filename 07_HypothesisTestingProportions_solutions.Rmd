...
```{r}
## contingency table
tab = table(dat.brca$age>50,dat.brca$grade)
...
2.  Divide the patients into two groups (ALL and AML) and run a t-test and a wilcoxon test on this gene. Is there a difference between the p-values? What did you expect? *Hint*: You can use the `i.all` vector to subset the patients.
...
plot(-log10(P[1,]),-log10(P[2,]),pch=20,xlab = '-log10(T-test)',ylab='-log10(Wilcoxon)');abline(0,1)
```
...
**SOLUTION** one categorical variable (social science / natural science) but the grades are continuous variables.
So t-test or Wilcoxon test would be appropriate!
