...
**Recommendation:** in order to avoid going through several excercise sheets in the future to perform the data cleaning, create now a R file where you save all of the code that we use to manipulate our data.\ 
...
2.  Use the numerical dataframe "dat.num", and apply the function `cor()` **directly** on this data frame: `cor(dat.num)` . What happens? What is the output? Store the result of this command in a variable named "all.cor".\ 
Plot a heatmap of these results using the `pheatmap()` function from the "pheatmap" package. Remember that you first have to activate this package using `library("pheatmap")`.
