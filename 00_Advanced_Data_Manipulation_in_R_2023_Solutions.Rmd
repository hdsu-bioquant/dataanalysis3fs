...
Instead of using built-in functions, we can also write our own little function and combine it with `sapply()`: 
...
# 1.
apply(M, 1, mean) # mean of every row

# 2. 
apply(M, 2, mean) # mean of every column

#3. 
which(M > 0) # positions of all positive elements
M[which(M > 0)] #values of all positive elements
```
...
sapply(y, function(x) {ifelse(x > 0, sqrt(x), 0)})
```
