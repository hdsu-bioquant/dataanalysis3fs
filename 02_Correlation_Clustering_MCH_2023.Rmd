...
Next, we go through each row and sum up all missing values (NA values).
...
Apply() is used with the parameter 1 for rows and the parameter 2 for columns. Since we'd like to calculate the number of NAs per row, we use the parameter 1 in the functions. The sum of NAs is stored in the variable rmv.rows - our next aim beeing to remove those rows with NAs.
```{r}
