...
Would could at first check how the weight values are distributed between males vs. females
...
# Select the weigth values accordingly
...
-   there is a significant difference in the mean of the weights between the two groups (two-sample, two-tailed test)
-   male have a significantly higher mean weight than females (two-sample, one-tailed test)
...
Here, if we want to test whether the mean weight of the 10 chocolate bars is **different** from the expected weight of 100 g, we want to perform a *two-tailed* test and use the alternative *two.sided*.  

The hypotheses are the following: 
> H0: the 10 chocolate bars have a mean weight **equal** to 100 g.
> H1: the 10 chocolate bars habe a mean weigth **different** from 100 g.
...
Regarding the mean weight of the males, we would like to check whether males have a mean weight that is significantly lower than 180. Here as well, we will perform a one-sample test, with mu = 180. However, we will perform a one-tailed test using the alternative option **less**.  
The hypotheses can be formulated as:
> H0: males have a mean weight that is **equal or greater** 180.
> H1: males have a mean weigth that is **less** than 180.
...
The hypotheses can be formulated as:
> H0: males have a mean weight that is **equal** to the mean weight of females.
> H1: males have a mean weigth that is **different** from the mean weight of females.

How would you interpret the result with alpha = 0.05?
```{r}
t.test(weigth.men,weight.women)
...
> H0: males have a mean weight that is **equal or lower** to the mean weight of females.
> H1: males have a mean weigth that is **higher** from the mean weight of females.

How would you interpret the result with alpha = 0.05?
```{r}
t.test(weigth.men,weight.women, alternative = "greater")
```
...
```{r}
t.test(weigth.men,weight.women, alternative = "two.sided")$p.value
t.test(weigth.men,weight.women, alternative = "greater")$p.value
...
> For the exercises, consider the data as normally distributed.

------------------------------------------------------------------------

### Exercise 1

Consider the following graph, formulate the hypotheses H0 and H1 and perform a test. Interpret the result using alpha = 0.05.
...
```
> Solution:
H0: the mean heigth value for the women group is **equal or larger** than the mean height for the men group.
H1: the mean heigth value for the women group is **smaller** than the mean height for the men group.
...
> With alpha = 0.05, the H0 hypothesis can be rejected (p-value < alpha)

------------------------------------------------------------------------

### Exercise 2

Calculate the mean age of the men. Compare it to age = 50. Formulate the hypotheses H0 and H1. Perform a test and interpret the result (alpha = 0.05).
...
> H0: the mean age of men is **greater or equal** to 50.
> H1: the mean age of men is **smaller** than 50.
...
> With alpha = 0.05, the H0 hypothesis cannot be rejected (p-value >= alpha)
...
3.1 Select the two groups according to the location. To do so, check the result of "unique(dat$location)" and create two vectors accordingly.
```{r}
unique(dat$location)
#
...
3.2 Calculate the mean values of the numerical parameters for each group (ex: age, height, weight, waist, hip, ...). Maybe you'd like to create first a table with numeric columns only using c(2,3,4,5,6,8,10,11,13,14,17,18).
```{r}
...
3.3 Select one of these, formulate the H0 and H1 hypotheses and perform a test. Interpret the result (alpha = 0.05).

> H0: the mean chol value of the Buckingham groups is **equal** than the mean chol value of the Louisa group.
> H1: the mean chol value of the Buckingham groups is **different** from the mean chol value of the Louisa group.

```{r}
# two-sample t.test, two-tailed (alternative = two.sided)
t.test(num.dat$chol[rows.Buckingham],num.dat$chol[rows.Louisa])
```
...
```{r}
sapply(num.dat,shapiro_test, na.rm = TRUE)
```
...
Here, with a QQ-plot:
```{r}
qqnorm(dat$bp.1d,main="weight"); qqline(dat$bp.1d)
```

Now, we can perform a test on this parameters, using the male and female groups. Considering the boxplots as seen below, we formulate the following hypotheses:
> H0: the bp.1d mean value of the men group is **equal** to the pb.1d mean value of the women group.
> H1: the bp.1d mean value of the men group is **different** from the pb.1d mean value of the women group.

```{r}
boxplot(list(dat$bp.1d[rows.men],dat$bp.1d[rows.women]),main="bp.1d values", names = c("men","women"))
```

Accordingly, we perform a two-sample t.test (two-tailed). The result will be interpreted using alpha = 0.05.
...
If the data is *NOT* normally distributed, a **Wilcoxon test** can be performed.
Compare here the results for the t-test as compared to the wilcoxon test applied on the parameter "height" for the men and women groups.

```{r}
# two-tailed tests - alternative = "two.sided" (per default in the functions)
t.test(dat$height[rows.men],dat$height[rows.women])$p.value
wilcox.test(dat$height[rows.men],dat$height[rows.women])$p.value
```

Below, you will find the comparison for all the numerical parameters (remember, they were not normally distributed according to the *Shapiro-Wilk* test).

```{r}
pvalue.compare = sapply(1:ncol(num.dat), function(x) {
  ttest = t.test(num.dat[rows.men,x],num.dat[rows.women,x])$p.value
  wilcoxtest = wilcox.test(num.dat[rows.men,x],num.dat[rows.women,x])$p.value
  c(ttest,wilcoxtest)
})
rownames(pvalue.compare) = c("t.test","Wilcoxon.test")
colnames(pvalue.compare) = colnames(num.dat)
pvalue.compare
```

Note that for the parameters of our dataset the conclusions are similar for alpha = 0.05, except for the "weight".
