...
We will load the dataset into R and have a sneak peek into how it looks like with the console.
...
2.  Create a new variable called `bmi` (Body Mass Index) in the dataset using the formula: BMI = weight (in pounds) / height\^2 (in inches) × 703. Add this as a new column to `dat`.
3.  Use the `table()` function to count how many patients come from each location. Which location has the most patients?
...
1.  Create a scatter plot of `weight` vs `height` using the `plot()` function.
    Do you see a relationship between these variables?

2.  Use the `cor()` function to calculate the correlation coefficient between weight and height.
    How strong is this relationship?

3.  Create side-by-side boxplots comparing cholesterol levels (`chol`) between males and females using `boxplot(chol ~ gender, data = dat)`.
    What differences do you observe?
    Try with other variables!

4.  Color the points in your scatter plot from question 1 by gender using the `col` parameter. 
    *Hint: you can use `col = as.numeric(as.factor(dat$gender))`*.
...
1.  Create a categorical variable called `age_group` that divides patients into three categories: "young" (age \< 40), "middle" (40 ≤ age \< 60), and "senior" (age ≥ 60).
    ...
    *Hint: use nested `ifelse()`statements or the `cut()` function*.
