...
```{r}
dat = read.delim('https://tinyurl.com/y4fark9g', stringsAsFactors = FALSE) # Load the dataset
...
To perform this cleanup, we need a couple of important functions, that we will first discuss. If you went through "00_Advanced_Data_Manipulation_in_R.Rmd" from last week, you might already be familiar with some of these functions:
...
`which()` is used on a data structure and generates a data structure with the positions of the values that satisfy a condition you specify. Let's look at an example.
...
See? We selected all names form the "Players" vector that correspond to more than three points in the "Points" vector.\ 
The type of condition can also vary, depending also on the data type in your dataset. Just remember that it must have a **boolean** output (TRUE/FALSE) to be used in `which()`, e.g. `is.na()` (see below) can be used inside a `which()` function.

## apply

`apply()` is used to apply a function to an entire dataset at once and outputs a vector of results. Later, we will use it like this:
...
apply(Mat, 1, function(x) {mean(x)}) # Calculate the mean of each row
```
...
This is a vector of **logical** (or boolean) values. Let's go one step further:
...
The rows are just numbered, without names. We are going to rename the rows using the column "id".
...
The ordering and selection of columns looks right, however it seems that there are certain rows that have missing values (like `glyhb` column has 13 `NA` values). Lets remove all rows with any missing value: 
We go through each row and sum up all missing values (NA values).
...
`apply()` is used with the parameter 1 for rows and the parameter 2 for columns. Since we'd like to calculate the number of NAs per row, we use the parameter 1 in the functions. The sum of NAs is stored in the variable rmv.rows - our next aim being to remove those rows with NAs.
```{r}
rmv.rows = apply(dat, 1, function(x) {sum(is.na(x))}) 
```
...
> What should be the expected length of this vector? Think about the dimensions of the dataframe "dat"
...
For these rows, the variable rmv.rows will have values > 0. Correct?
...
Finally, we will turn all variables with `character` type (i.e. character strings) into **factors**. This will turn character variables into **nominal/ordinal** variable types. We will make the columns "Location" and "Gender" nominal and the "frame" ordinal.
...
> Do you remember the difference between nominal and ordinal variables? 
...
> Add the parameter `breaks = 50` in the above lines of code and see what happens. Try different values for `breaks` like `10, 20, 75, 100` and try to interpret the differences.
Is this a good or bad thing about histograms in general? 


### Density plots

For density plots, we use the `density()` function to estimate the probability density function for a given variable. With its return value, we can build the final density plot using the `plot()` function: 
...
You don't have to understand the code below in detail. Instead, you can just check the resulting plot. If you have questions, use `help()` for further information on a specific function or let us know.
...
*(expert):* Mark the median, first and third quantile with lines using `abline()`. Then mark median, first and third quantile for **female** patients in the same graph with a different color. What can you tell from the differences in these values?

3.  Use the `qqplot()` function to plot waist against hip, height against hip and chol against hip in a QQ-plot. Can you correctly interpret these plots? How are those distributions different?
