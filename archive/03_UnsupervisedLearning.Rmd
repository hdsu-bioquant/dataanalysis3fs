...
```{r}
<<<<<<< Updated upstream:03_UnsupervisedLearning.Rmd
pheatmap(cor.mat,annotation_row = all.aml.anno,annotation_col = all.aml.anno)
=======
pheatmap(cor.mat,annotation_row = brca.anno,
         show_rownames = FALSE, show_colnames = FALSE, clustering_method = 'complete')
>>>>>>> Stashed changes:archive/03_UnsupervisedLearning.Rmd
```
...
2.  Repeat the selection of top variable genes (apply the same quantile used to generate "brca.exp.topVar"), but using the median absolute deviation (or MAD, see lecture) using the `mad()` function instead of the `sd()` function, and store into a matrix `brca.ex.topMAD`
...
```{r}
pheatmap(brca.exp.topVar,annotation_col = brca.anno,show_rownames = FALSE, show_colnames = FALSE, clustering_method = 'complete',cluster_rows = FALSE)
```
>>>>>>> Stashed changes:archive/03_UnsupervisedLearning.Rmd
